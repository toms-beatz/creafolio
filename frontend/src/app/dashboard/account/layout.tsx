...
export default function AccountLayout({
    ...
    return (
        <div className="space-y-6">
            ...
            {/* Tab nav */}
            <nav className="flex gap-1 border-b border-dashed border-zinc-200 dark:border-zinc-800 pb-px overflow-x-auto -mx-4 px-4 md:mx-0 md:px-0 scrollbar-none">
                {accountNav.map((item) => (
                    <Link
                        key={item.id}
                        href={item.href}
                        className="rounded-t-lg px-3 sm:px-4 py-2 text-sm text-zinc-500 hover:text-zinc-900 dark:hover:text-white hover:bg-zinc-100 dark:hover:bg-zinc-900/50 transition-colors font-medium whitespace-nowrap"
                    >
                        {item.label}
                    </Link>
                ))}
