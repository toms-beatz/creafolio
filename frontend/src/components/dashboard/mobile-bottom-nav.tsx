...
export function MobileBottomNav({ items }: MobileBottomNavProps) {
    ...
    return (
        <nav aria-label="Navigation mobile" className="fixed bottom-0 inset-x-0 z-50 border-t border-dashed border-zinc-200 dark:border-zinc-800 bg-zinc-50/95 dark:bg-zinc-950/95 backdrop-blur-sm md:hidden">
                    ...
                    return (
                        <Link
                            ...
                            className={`flex flex-1 flex-col items-center justify-center gap-0.5 py-2.5 text-[10px] font-medium transition-colors ${active
                                ? 'text-[#ad7b60]'
                                : 'text-zinc-500 active:text-zinc-700 dark:active:text-zinc-300'
                                }`}
                        >
