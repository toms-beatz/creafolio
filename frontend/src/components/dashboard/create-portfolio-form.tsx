        ...
        <form action={formAction} className="flex flex-col gap-5">
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    {TEMPLATES.map((tmpl) => (
                        ...
                        >
                            ...
                            <div className="relative flex flex-row overflow-hidden bg-zinc-950">
                                ...
                                <div className="flex flex-1 flex-col items-center justify-center gap-2 p-4">
                                    ...
                                    {selectedTemplate === tmpl.id && (
                                        <span className="text-[#ad7b60] text-[10px] flex items-center gap-0.5"><Check className="h-3 w-3" /> sélectionné</span>
                                    )}
                                    {tmpl.premium && (
                            ...
                            {/* Info en bas */}
                            <div className={`flex flex-col gap-1.5 border-t border-dashed border-zinc-800 p-3 ${selectedTemplate === tmpl.id ? 'bg-[#ad7b60]/5' : 'bg-zinc-900/30'
                                }`}>
                                <p className="text-[10px] leading-relaxed text-zinc-500 line-clamp-2">
                                    {tmpl.description}
                                </p>
                                {tmpl.blocks.length > 0 && (
                                    <p className={`text-[9px] font-mono ${selectedTemplate === tmpl.id ? 'text-[#ad7b60]/60' : 'text-zinc-700'
                                        }`}>
            ...
            <div className="flex items-center gap-2 pt-1">
                <Button
                    type="submit"
                    loading={isPending}
                    className="flex-1 bg-[#ad7b60] text-zinc-950 hover:bg-[#d4a485]"
                >
