...
const freeFeatures: { label: string; included: boolean }[] = [
    ...
    { label: 'Sous-domaine blooprint.fr/username', included: true },
    { label: 'Templates de base', included: true },
    { label: 'Watermark Blooprint', included: false },
...
export function PricingContent({ isLoggedIn, plan, trialDaysLeft }: PricingContentProps) {
    ...
    return (
        <>
            {/* ── Hero ─────────────────────────────────────────────── */}
            <section className="relative overflow-hidden bg-zinc-950 pt-28 pb-16 lg:pt-36 lg:pb-20">
                ...
                <div
                    ...
                    style={{ background: 'radial-gradient(ellipse 70% 50% at 50% 30%, rgba(56,189,248,0.07) 0%, transparent 70%)' }}
                />

                <div className="relative mx-auto max-w-3xl px-4 text-center">
                    <CoordLabel text="[PLANS // 00.06]" className="mb-5 block text-sky-400/70 text-[10px] tracking-[0.25em]" />

                    <h1 className="text-4xl font-bold tracking-tighter text-white sm:text-5xl lg:text-6xl leading-[1.05] mb-4 animate-bp-fade-up">
                        Simple.{' '}
                        <span className="text-sky-400">Transparent.</span>
                    </h1>

                    <p className="text-zinc-400 text-lg mb-10 animate-bp-fade-up delay-100">
                        Commence gratuitement. Passe en Premium quand tu es prêt.
                    </p>

                    {/* Toggle mensuel / annuel */}
                    <div className="inline-flex items-center gap-2 rounded-full border border-dashed border-zinc-700 bg-zinc-900 p-1 animate-bp-fade-up delay-200">
                        <button
                        ...
                        >
                            Annuel
                            <span className="rounded-full bg-sky-400/20 px-1.5 py-0.5 font-mono text-[9px] text-sky-300">
                                −40%
                            </span>
                        </button>
                    </div>
            ...
            {/* ── Plans ────────────────────────────────────────────── */}
            <section className="relative bg-zinc-950 py-12 pb-24">
                <div className="mx-auto max-w-4xl px-4">
                    <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 animate-bp-fade-up delay-300">

                        {/* ─ Free ─ */}
                        <div className="relative flex flex-col rounded-2xl border border-dashed border-zinc-700 bg-zinc-900/40 p-7">
                            ...
                            <ul className="flex flex-col gap-3 mb-10 flex-1">
                                {freeFeatures.map((f) => (
                                    <li key={f.label} className="flex items-start gap-2.5 text-sm">
                                        {f.included ? (
                                            <span className="mt-0.5 text-xs text-sky-400/80 shrink-0"><Check className="h-3.5 w-3.5" /></span>
                                        ) : (
                        ...
                        {/* ─ Premium ─ */}
                        <div className="relative flex flex-col rounded-2xl border-2 border-sky-400/40 bg-zinc-900/60 p-7 shadow-lg shadow-sky-400/5">
                            {/* Badge populaire */}
                            ...
                            <ul className="flex flex-col gap-3 mb-10 flex-1">
                                {premiumFeatures.map((f) => (
                                    <li key={f.label} className="flex items-start gap-2.5 text-sm">
                                        <span className="mt-0.5 text-xs text-sky-400 shrink-0"><Check className="h-3.5 w-3.5" /></span>
                            ...
                            {isTrialActive && (
                                <div className="mb-4 rounded-lg border border-dashed border-sky-400/30 bg-sky-400/5 px-4 py-3 text-center">
                            ...
                            {isPremium ? (
                                <Button
                                    ...
                                    disabled={loading}
                                    className="w-full bg-sky-400 text-zinc-950 hover:bg-sky-300 shadow-lg shadow-sky-400/20 font-semibold"
                                >
            ...
            {/* ── Comparatif ───────────────────────────────────────── */}
            <section className="relative overflow-hidden bg-zinc-900/30 py-20 border-y border-dashed border-zinc-800">
                <div className="mx-auto max-w-3xl px-4">
                    ...
                    <div className="overflow-hidden rounded-xl border border-dashed border-zinc-700">
                        {/* Header */}
                        <div className="grid grid-cols-3 bg-zinc-900 px-5 py-3 text-xs font-mono text-zinc-500 uppercase tracking-widest">
                            <span>Fonctionnalité</span>
                            <span className="text-center">Free</span>
                            <span className="text-center text-sky-400/80">Premium</span>
                        </div>

                        {[
                            { label: 'Portfolios', free: '1', premium: '3 à 5' },
                            { label: 'Blocs par portfolio', free: '6 max', premium: 'Illimité' },
                            { label: 'URL', free: 'blooprint.fr/username', premium: 'Custom domain + sous-domaine' },
                            ...
                            <div
                                key={row.label}
                                className={`grid grid-cols-3 items-center px-5 py-3.5 text-sm border-t border-dashed border-zinc-800 ${i % 2 === 0 ? 'bg-zinc-950/30' : ''}`}
                            >
                                <span className="text-zinc-400">{row.label}</span>
                                <span className="text-center text-zinc-500">{row.free}</span>
                                <span className="text-center text-sky-300 font-medium">{row.premium}</span>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <section className="relative overflow-hidden bg-zinc-950 py-20">
                ...
                <div className="relative mx-auto max-w-2xl px-4">
                    ...
                    <div className="flex flex-col divide-y divide-dashed divide-zinc-800 rounded-xl border border-dashed border-zinc-700 overflow-hidden">
                        {faq.map((item, i) => (
                            <div key={i} className="bg-zinc-900/40">
                                <button
                                    onClick={() => setOpenFaq(openFaq === i ? null : i)}
                                    className="flex w-full items-center justify-between gap-4 px-5 py-4 text-left text-sm font-medium text-zinc-200 hover:text-white transition-colors"
                                >
                                    <span>{item.q}</span>
                                    <span
                                        className="shrink-0 font-mono text-sky-400/70 transition-transform duration-200"
                                ...
                                {openFaq === i && (
                                    <p className="px-5 pb-4 pt-1 text-sm text-zinc-400 leading-relaxed border-t border-dashed border-zinc-800">
                                        {item.a}
                                    </p>
                                )}
                            </div>
            ...
            {/* ── CTA final ────────────────────────────────────────── */}
            <section className="relative overflow-hidden bg-zinc-900/40 border-t border-dashed border-zinc-800 py-20">
                <div
                    className="pointer-events-none absolute inset-0"
                    style={{ background: 'radial-gradient(ellipse 60% 80% at 50% 100%, rgba(56,189,248,0.06) 0%, transparent 70%)' }}
                />

                <div className="relative mx-auto max-w-xl px-4 text-center">
                    <CoordLabel text="[CTA // 00.09]" className="mb-5 block" />
                    <h2 className="text-2xl font-bold text-white tracking-tight mb-3 sm:text-3xl">
                        Prêt à construire ton portfolio ?
                    </h2>
                    <p className="text-zinc-400 mb-8 text-base">
                        Rejoins Blooprint gratuitement.{' '}
                        <span className="text-zinc-300">Essai Premium 7 jours inclus.</span>
                    </p>
                    ...
                    <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
                        <Button
                            asChild
                            size="lg"
                            className="w-full sm:w-auto bg-sky-400 text-zinc-950 hover:bg-sky-300 shadow-lg shadow-sky-400/20 font-semibold px-8"
                        >
                        ...
                        <Button
                            ...
                            asChild
                            size="lg"
                            className="w-full sm:w-auto border-dashed border-zinc-700 text-zinc-300 hover:bg-zinc-900 hover:text-white"
                        >
